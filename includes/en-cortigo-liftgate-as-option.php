...
if(!class_exists("En_Cortigo_Liftgate_As_Option"))
{
    class En_Cortigo_Liftgate_As_Option{
        
        /**
         * set flag
         * @var string type 
         */
        public $cortigo_freights_as_option;
        ...
        {
            $this->cortigo_freights_as_option = get_option("cortigo_freights_liftgate_delivery_as_option");
         ...
         * @return array type
         */
        public function cortigo_freights_update_carrier_service( $post_data )
        {
            if(isset($this->cortigo_freights_as_option) && ($this->cortigo_freights_as_option == "yes"))
            {
                (isset($post_data['carriers']['cortigo']['api']['accessorial'])) ? $post_data['carriers']['cortigo']['api']['accessorial'][] = "LFTGATORIG" : "";
         ...
         * @return array type
         */
        public function update_parse_cortigo_freights_output( $surcharges )
        {   
         ...
         * @return array
         */
        public function update_rate_whn_as_option_cortigo_freights( $rate )
        {
            if(isset($rate) && (!empty($rate))){
                $rate = apply_filters("en_woo_addons_web_quotes" , $rate , en_woo_plugin_cortigo_freights);
                
                $label_sufex = (isset($rate['label_sufex'])) ? $rate['label_sufex'] : array();  
                $label_sufex = $this->label_R_cortigo_freights($label_sufex);
                $rate['label_sufex'] = $label_sufex;
                
                if(isset($this->cortigo_freights_as_option,$rate['grandTotalWdoutLiftGate']) && 
                        ($this->cortigo_freights_as_option == "yes") && ($rate['grandTotalWdoutLiftGate'] > 0) )
                { 
         ...
         * @return array type
         */
        public function filter_label_sufex_array_cortigo_freights($result)
        {
         ...
         * @return array type
         */
        public function label_R_cortigo_freights($label_sufex)
        {
    ...
    new En_Cortigo_Liftgate_As_Option();
}

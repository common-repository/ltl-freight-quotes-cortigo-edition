...
if (!function_exists('warehouse_template')) {
    function warehouse_template($action = FALSE)
    {
        ...
        $warehous_list = $wpdb->get_results(
            "SELECT id, city, state, zip, country, location , enable_store_pickup , fee_local_delivery , suppress_local_delivery , miles_store_pickup , match_postal_store_pickup , checkout_desc_store_pickup , enable_local_delivery , miles_local_delivery , match_postal_local_delivery , checkout_desc_local_delivery
        ...
        <a href="#en_wd_add_warehouse_btn" onclick="en_wd_add_warehouse_btn()" title="Add Warehouse"
           class="en_wd_add_warehouse_btn <?php echo $multi_warehouse_disabled; ?>" name="avc">Add</a>
        ...
        <div class="warehouse_text">
            <p>Warehouses that inventory all products not otherwise identified as drop shipped items. The warehouse with
                the lowest shipping cost to the destination is used for quoting purposes.</p>
        </div>
            ...
            if (count($warehous_list) > 0) {
                ...
                foreach ($warehous_list as $list) {
                    ...
                    <tr class="<?php echo (strlen($tr_disabled_me) > 0 && $count != 0) ? $tr_disabled_me : ""; ?>"
                        ...
                        <td class="en_wd_warehouse_list_data">
                            <a href="javascript(0)"
                               onclick="return en_wd_edit_warehouse(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img
                                        src="<?php echo plugins_url(); ?>/ltl-freight-quotes-cortigo-edition/includes/warehouse-dropship/wild/assets/images/edit.png"
                                        title="Edit"></a>
                            <a href="javascript(0)"
                               onclick="return en_wd_delete_current_warehouse(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img
                                        src="<?php echo plugins_url(); ?>/ltl-freight-quotes-cortigo-edition/includes/warehouse-dropship/wild/assets/images/delete.png"
                                        title="Delete"></a>
                        </td>
                    </tr>
                    <?php
...
if (!function_exists('dropship_template')) {
    function dropship_template($action = FALSE)
    {
        ...
        <br><?php echo $add_space; ?>
        <div class="warehouse_text">
            <p>Locations that inventory specific items that are drop shipped to the destination. Use the product's
                settings page to identify it as a drop shipped item and its associated drop ship location. Orders that
                include drop shipped items will display a single figure for the shipping rate estimate that is equal to
                the sum of the cheapest option of each shipment required to fulfill the order.</p>
        </div>
            ...
            if (count($dropship_list) > 0) {
                ...
                foreach ($dropship_list as $list) {
                    ...
                    <tr class="<?php echo (strlen($tr_disabled_me) > 0 && $count != 0) ? $tr_disabled_me : ""; ?>"
                        ...
                        <td class="en_wd_dropship_list_data">
                            <a href="javascript(0)"
                               onclick="return en_wd_edit_dropship(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img
                                        src="<?php echo plugins_url(); ?>/ltl-freight-quotes-cortigo-edition/includes/warehouse-dropship/wild/assets/images/edit.png"
                                        title="Edit"></a>
                            <a href="javascript(0)"
                               onclick="return en_wd_delete_current_dropship(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img
                                        src="<?php echo plugins_url(); ?>/ltl-freight-quotes-cortigo-edition/includes/warehouse-dropship/wild/assets/images/delete.png"
                                        title="Delete"></a>
                        </td>
                    </tr>
